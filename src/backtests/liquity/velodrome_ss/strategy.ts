...
class SingleSidedVelodrome {
  ...
  public async process(
    ...
    if (!this.pos) {
      ...
      let [pos, idle] = await velodrome.addLiquidity(
        this.poolSymbol,
        this.initial,
        1,
      );
    ...
    const strategies = [
      {
        ...
        name: 'A: sAMM-USDC/LUSD',
        pool: 'sAMM-USDC/LUSD',
      },
      {
        initialInvestment: 10_000,
        name: 'A: sAMM-LUSD/MAI',
        pool: 'sAMM-LUSD/MAI',
      },
      {
        initialInvestment: 10_000,
        name: 'A: sAMM-USD+/LUSD',
        pool: 'sAMM-USD+/LUSD',
      },
    ];
    this.strategies = strategies.map(
      (s) => new SingleSidedVelodrome(s.name, s.pool, s.initialInvestment),
    );
  }
