...
import { AAVEPositionManager, AAVEPosition } from "../../../lib/protocols/AavePositionManager.js";
...
class HedgedVelodrome {
	...
	private calcLenderAmounts(totalAssets: number, data: VelodromeSnaphot) {
        ...
        const borrow = borrowInUSD / pool.tokens[this.tokenIndex == 0? 1:0].price
        return {borrow, lend}
    }
	...
	private getSpotPriceInWant(data: VelodromeSnaphot) {
		...
		const price = this.tokenIndex == 0 ? (reserve0/reserve1) : (reserve1/reserve0)
		return price
	}

	public borrow(data: VelodromeSnaphot) {
		const pool = this.pool(data)
		const borrow = (this.aave.borrowed(pool.tokens[this.tokenIndex == 0 ? 1 : 0].symbol))
		return borrow
	}
	...
	private async openFirstPosition(
		...
		let amountWant = ((this.initial/pool.tokens[this.tokenIndex].price) - result.lend)
		...
		this.aave.borrow(pool.tokens[this.tokenIndex == 0 ? 1 : 0].symbol, result.borrow)
		let [pos, idle] = await velodrome.addLiquidityHedged(this.poolSymbol, amount0, amount1)
		this.pos = pos
		this.idle = idle
		this.start = data.timestamp
		this.lastHarvest = this.start
	}
	public async rebalanceDebt(mgr: VelodromePositionManager, aave: AAVEPositionManager, data: VelodromeSnaphot) {
		this.rebalanceCount++
		...
		let [pos, idle] = await mgr.addLiquidityHedged(this.poolSymbol, amount0, amount1)
		this.pos = pos
		this.idle = idle
	...
	private async checkRebalance(mgr: VelodromePositionManager, aave: AAVEPositionManager, data: VelodromeSnaphot){
	...
	private estTotalAssets(data: VelodromeSnaphot) {
		...
		const total = this.claimed + lpUSD + this.idle + lent - debt - this.gasCosts
		return total
	}
...
export class HedgedVelodromeStrategy {
    ...
    constructor() {
		const strategies = [
			{ initialInvestment: 100_000, name: 'A: vAMM-WETH/LUSD', pool: 'vAMM-WETH/LUSD', collatRatio: 0.6, debtRatioRange: 0.02, tokenIndex: 1 },
		...
		this.strategies = strategies.map(s => new HedgedVelodrome(s.name, s.pool, s.initialInvestment, s.collatRatio, s.debtRatioRange, s.tokenIndex))
    }
