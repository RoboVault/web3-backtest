...
export class JoesV2AutoPoolStrategy {
  ...
  private balanceBase(bins: Bin[]) {
    return this.position.map(pos =>{
      const bin = bins.find(e => e.id === pos.bin)
      if (!bin) {
        throw new Error('Missing bin in data. More data is required for this strategy')
        return 0
      }
      ...
      return bin.reserveX === 0 
        ? 0
        : bin.reserveX * pos.supply / binSupply
    }).reduce((a, b) => a + b, 0)
  }
  ...
  private depositIntoActiveBin(price: number, deposit: { base: number, quote: number }, bin: Bin): number {
  ...
  private createSpotPosition(data: JoesV2PoolSnapshot, amountInQuote: number) {
    ...
    const activeBinBase = quoteValuePerBin / (price + activeBin.reserveX / activeBin.reserveY)
    ...
    for (let id = activeBinId - binRange; id < activeBinId; id++) {
      ...
      const getPosition = () => {
        ...
        if (bin.reserveY === 0) {
          ...
          const supply = (quoteSplit / bin.reserveY) * this.supplyFromHex(bin.supply) 
          if (isNaN(supply)) {
            console.log(quoteSplit, bin.reserveY, this.supplyFromHex(bin.supply))
    ...
    for (let id = activeBinId + 1; id <= activeBinId + binRange; id++) {
      ...
      const getPosition = () => {
        ...
        if (bin.reserveX === 0) {
          ...
          const supply = (baseSplit / bin.reserveX) * this.supplyFromHex(bin.supply) 
          if (isNaN(supply)) {
            console.log(baseSplit, bin.reserveX, this.supplyFromHex(bin.supply))
