...
export class StructJoesV2StrategyRunner {
  ...
  public async startNewStartForPool(pool: string) {
    this.strategies.push(new StructJoesV2Strategy({
      initialValue: 1000,
      period: PERIOD,
      fixedApr: 0.1,
      fixedToken: 'base',
  ...
  public async onData(snapshot: JoesV2Snaphot) {
    if (!snapshot.data.joes)
      return console.log('missing joes data', snapshot);
    ...
    for (const symbol of POOLS) {
      ...
      const strats = this.strategies.filter((s) => s.options.pool.symbol === symbol)
