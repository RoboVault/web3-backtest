...
export class VelodromePosition {
	...
	static async openHedged(data: VelodromePoolSnapshot, amount0: number, amount1: number): Promise<[VelodromePosition, number]> {
		// if(tokenIndex > 1)
		// 	throw new Error('Invalid Token Index!')
	...
	public async close(data: VelodromePoolSnapshot, stable: boolean) {
		...
		const tokenAmounts = await VelodromePosition.calc_withdraw_one_coin(data, lpTokensBN, stable)		
	...
	public async pendingRewards(data: VelodromePoolSnapshot) {
		...
		const posVeloEarned = (this.lpAmount * (thisRewardsPerToken-lastRewardsPerToken)) / (10**18)
...
export class VelodromePositionManager {
	...
	public async addLiquidityHedged(
		...
		const pair = this.lastData.data.velodrome.find(p => p.symbol === symbol)!
		let [pos, idle] = await VelodromePosition.openHedged(pair, amount0, amount1)
		this.positions.push(pos)
		return [pos, idle]
    }
