...
export class CurvePosition {
  ...
  private constructor(public data: CurvePoolSnapshot, public lpAmount: number) {
  ...
  static contract(data: CurvePoolSnapshot) {
    ...
    if (data.symbol === 'bLUSDLUSD3-f')
      ...
      return new ethers.Contract(
        data.pool,
        CurveStableSwapAbi as any,
        provider,
      );
  }

  static calc_token_amount(
    data: CurvePoolSnapshot,
  ...
  static calc_withdraw_one_coin(
    ...
    const curve = CurvePosition.contract(data);
    if (data.symbol === 'bLUSDLUSD3-f')
      return curve.calc_withdraw_one_coin(amount, index, {
        blockTag: data.block,
      });
    else
      return curve.calc_withdraw_one_coin(amount, index, {
  ...
  static async open(data: CurvePoolSnapshot, amounts: number[]) {
    ...
    const amountsBigInt = amounts.map((a, i) =>
      toBigNumber(a, data.tokens[i].decimals),
    );

    const lpAcountBigInt = await CurvePosition.calc_token_amount(
      data,
      amountsBigInt,
    );
  ...
  public async close(data: CurvePoolSnapshot, index: number) {
    ...
    const tokenAmount = await CurvePosition.calc_withdraw_one_coin(
      data,
      lpTokensBN,
      index,
    );
    ...
    this.lpAmount = 0;
    const amountUSD =
      toNumber(tokenAmount, data.tokens[index].decimals) * price;
    return amountUSD;
  }
  ...
  public pendingRewards(data: CurvePoolSnapshot) {
    ...
    const crvRewards =
      data.crvRate * data.gaugeRelativeWeight * period * positionRelativeWeight;
