...
export const calculateL = (pDeposit: number, vDeposit: number, pLower: number, pUpper: number) => {
    if (pDeposit < pLower)
        ...
        return vDeposit / ( pDeposit * (1 / Math.sqrt(pDeposit) - 1 / Math.sqrt(pUpper) ) + Math.sqrt(pDeposit) - Math.sqrt(pLower) )
}
...
export const  getXReal = (pCurrent: number, pLower: number, pUpper: number, L: number) => {
    if (pCurrent < pLower)
        return getXMax(pLower, pUpper, L)
    else if (pCurrent > pUpper)
        return 0
    else   
        return (L / Math.sqrt(pCurrent)) - getXUpper(pLower, pUpper, L)
}
export const  getYReal = (pCurrent: number, pLower: number, pUpper: number, L: number) => {
    if (pCurrent < pLower)
        return 0
    else if (pCurrent > pUpper)
        return getYMax(pLower, pUpper, L)
    else   
        return (L * Math.sqrt(pCurrent)) - getYLower(pLower, pUpper, L)
}
    
export const getXLP = (pCurrent: number, pLower: number, pUpper: number, L: number) => {
    return (getXReal(pCurrent, pLower, pUpper, L) * pCurrent)
}
    
export const getYLP = (pCurrent: number, pLower: number, pUpper: number, L: number) => {
    return (getYReal(pCurrent, pLower, pUpper, L) * pCurrent)
}
   
export const getVLP = (pCurrent: number, pLower: number, pUpper: number, L: number) => { 
